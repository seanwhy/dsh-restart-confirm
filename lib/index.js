 ...
 *   GET  /dsh-health   — liveness probe polled by the client status dot.
 ...
 * supervisor).
 *
 * Why native setTimeout / direct shell.start in the handler: a webServer
 * handler is a plain Node HTTP callback outside any Cordis fiber, so
 * `ctx.effect`-backed timers are silently dropped. We therefore avoid host
 * timers entirely — the helper script carries every delay.
 */
...
export function apply(ctx, config) {
  ...
  const disposeRoute = ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const relaunch = buildRestartCommand(config)
