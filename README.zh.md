...
| 待重启提示 | 默认隐藏；每 5 秒读取插件市场操作状态，只有其他插件处于待安装/更新/移除（需要重启）时才显示绿色点 |

## 功能特性

- **二次确认** —— 点击按钮先弹出第一个对话框，再弹出最终警告框。只有第二次确认才会发送请求，两个对话框都可以取消。
- **自适应位置与尺寸** —— 通过 DOM 放置把按钮钉在侧边栏折叠按钮旁：展开时位于"收起侧边栏"左侧，56px 收起栏中位于"打开侧边栏"上方。展开行图标为 16px，收起栏为 18px；MutationObserver 保证在重渲染和折叠/展开切换后位置始终正确。
- **自动重新拉起并立即刷新** —— 杀掉进程后，插件用相同参数、相同工作目录重新启动 DSH。客户端比较健康接口返回的进程 bootId，新进程就绪后立即刷新页面，不再固定等待 15–20 秒。
...
| 层 | 文件 | 作用 |
| --- | --- | --- |
| Host | `lib/index.js` | 在 webServer 注册带进程 `bootId` 的 `GET /dsh-health` 与 `POST /restart-dsh`；启动独立后台助手：等待 → SIGTERM →（SIGKILL）→ 用原始 argv 重新拉起 |
| Client | `lib/client.js` | 纯 vanilla（无 React）客户端，通过 DOM 放置固定尺寸图标；仅在插件市场报告待重启插件时显示绿点；等待 bootId 变化并在新服务健康后立即刷新 |
| Bundle | `cordis.patch.yml` | 挂载两半的加载行 |

处理函数**先回复浏览器**再执行杀进程；杀进程/重新拉起的延迟由助手脚本承担。客户端不猜测重启完成时间，而是轮询健康接口，bootId 变化且新服务就绪后立即刷新。
